        ...
        <tbody>
          {tableData.map((task) => (
            <tr key={task._id} className="border-t border-gray-200">
              <td className="my-3 px-4 text-gray-700 text-[13px] line-clamp-1 overflow-hidden ">
                {task.title}
              </td>
              <td className="px-4 py-4">
                {" "}
                <span
                  className={`px-3 py-2 text-xs rounded inline-block ${getStatusBadgeColor(
                    task.status
                  )} `}
                >
                  {task.status}
              ...
              <td className="px-4 py-4">
                {" "}
                <span
                  className={`px-3 py-2 text-xs rounded inline-block ${getPriorityBadgeColor(
                    task.priority
                  )} `}
                >
                  {task.priority}
